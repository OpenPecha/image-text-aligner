...
const createSampleHistory = (_taskId: string, status: TaskStatus): TaskHistoryEntry[] => {
...
export const tasks: Task[] = [
  // Pending tasks
  {
    ...
    history: createSampleHistory('task_001', TaskStatus.Pending),
  ...
  },
  {
    ...
    correctedText: '',
    status: TaskStatus.Pending,
    history: createSampleHistory('task_002', TaskStatus.Pending),
  ...
  // Awaiting Final Review
  {
    ...
    reviewerId: 'user_reviewer_001',
    reviewerName: 'Riley Reviewer',
    history: createSampleHistory('task_007', TaskStatus.AwaitingFinalReview),
    createdAt: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000),
    updatedAt: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000),
  },
  // Completed
  {
    ...
    history: createSampleHistory('task_008', TaskStatus.Completed),
  ...
  // Rejected task (sent back for rework)
  {
    ...
    reviewerName: 'Riley Reviewer',
    history: [
      ...createSampleHistory('task_009', TaskStatus.InReview),
      {
        id: generateId(),
...
// Helper to get tasks assigned to a user
export const getTasksAssignedTo = (userId: string): Task[] => {
  return tasks.filter(task => task.assignedTo === userId)
}

// Helper to get tasks for review
export const getTasksForReview = (): Task[] => {
...
// Helper to get tasks for final review
export const getTasksForFinalReview = (): Task[] => {
  return tasks.filter(task => 
